...
// ---------------------------------------------------------------- 价格序列
// 画图用的历史曲线。fetchHourChange 其实已经把 24h 序列拉回来了，但只拿它算了个
// 百分比就把序列丢了——这里单独缓存一份完整序列，给前端画走势用。
// 同样带 TTL：CoinGecko 免费层限流很凶，不能每次请求都打上游。

let seriesCache = { key: "", at: 0, data: null };
const SERIES_TTL_MS = 5 * 60 * 1000;

// Coinbase Exchange 的 K 线粒度（秒）。按窗口挑，保证点数落在 100~300 之间：
// 太密传起来浪费，太疏看不出走势。
const CB_GRAN = { 1: 300, 7: 3600, 30: 21600 };

/**
 * Coinbase Exchange 兜底序列。CoinGecko 免费层限流很凶（一天跑几次回测就会触发），
 * 而序列只有它能给——不补这一段的话，一被限流图就整个空了。
 * 返回按时间升序；Coinbase 给的是降序，必须排序。
 */
async function fetchCbSeries(symbol, days) {
  const gran = CB_GRAN[days] || 300;
  const d = await getJson(
    CB_EXCHANGE + "/products/" + symbol + "-USD/candles?granularity=" + gran, 10000
  );
  if (!Array.isArray(d)) return null;
  const out = [];
  for (const c of d) {
    // 每条形如 [time(秒), low, high, open, close, volume]
    if (Array.isArray(c) && Number.isFinite(c[0]) && Number.isFinite(c[4])) {
      out.push({ ts: c[0] * 1000, price: c[4] });
    }
  }
...
/**
 * 各标的的价格序列（按时间升序）。days 支持 1 / 7 / 30。
 *
 * 返回 { series, source }：series 里拿不到的标的直接缺席（而不是给空数组）——
 * 「没有数据」和「有数据但零个点」是两回事，调用方能区分才不会画出一条假的平线。
 * source 记的是序列实际来自哪个源，好让界面说清楚这图是哪儿来的。
 */
async function getChartSeries(days = 1) {
  const now = Date.now();
  const key = String(days);
  if (seriesCache.data && seriesCache.key === key && now - seriesCache.at < SERIES_TTL_MS) {
  ...
  for (const s of SYMBOLS) {
    ...
    try {
      const d = await getJson(
        CG_CHART + "/" + IDS[s] + "/market_chart?vs_currency=usd&days=" + days, 10000
      );
      const raw = (d && d.prices) || [];
      const clean = [];
      for (const p of raw) {
        if (Array.isArray(p) && Number.isFinite(p[0]) && Number.isFinite(p[1])) {
          clean.push({ ts: p[0], price: p[1] });
        }
      }
      if (clean.length >= 2) { pts = clean; sources.add("coingecko"); }
    } catch (_) { /* 落到兜底 */ }

    if (!pts) {
      try {
        const cb = await fetchCbSeries(s, days);
        if (cb) { pts = cb; sources.add("coinbase"); }
      } catch (_) { /* 这个标的这次拿不到，让它缺席 */ }
    }
  ...
  const data = {
    series: out,
    source: sources.size === 0 ? "unavailable" : (sources.size > 1 ? "mixed" : [...sources][0]),
  };
  seriesCache = { key, at: now, data };
  return data;
}
...
module.exports = { getSnapshot, getChartSeries, SYMBOLS, series };
