...
固定同一批样本（35 个时点 × 2 标的）、同一套交易规则，**只换 criteria 的措辞**：

```bash
node tools/compare-criteria.js --samples 36 --days 7
```
...
**1 · 是措辞问题，不是「模型不会说 buy」。** 同一个模型、同一批行情，`strict` 和 `reversion` 一次 buy 都没有，`trend` 说了 29 次。差别全在 criteria 怎么写。我第一轮回测后推断「模型本身几乎不输出 buy」——**这个推断是错的**，是那句 buy 写得没人能达到。

**2 · 但能成交 ≠ 有 edge。** `trend` 的 +2.17% 只比随机信号的 90 分位（+2.06%）高一点点，**落在噪声区间内**。而且它跑输买入持有（+2.72%）。10 笔成交、35 个样本——**完全不足以判断胜率 60% 是不是运气**。

所以线上默认切到了 `trend`（唯一会真的产生成交、能让买卖点功能显示出来的那套），但页面和这份 README 都把上面这些数字摆在明处。**不要看到「有成交」就以为策略有效。**

**自己跑**：

```bash
export AI_GATEWAY_API_KEY=vck_xxx
node tools/compare-criteria.js --samples 36 --days 7   # 三套对照 + 随机基线，约 6 分钟
